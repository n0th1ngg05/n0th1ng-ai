...
# Ordinal suffixes are already spoken correctly by Kokoro as-is (e.g.
# "21st" -> "twenty first"), so no rewriting needed there — left out
# intentionally to avoid double-processing.

# Common small-number fraction words, used for "1/2" -> "one half" etc.
_FRACTION_WORDS: dict[str, str] = {
    "1/2": "one half",
    "1/3": "one third",
    "2/3": "two thirds",
    "1/4": "one quarter",
...
def _normalize_text_for_tts(text: str) -> str:
    ...
    "10 km/h" -> "10 kilometers per hour", "34.25" -> "34 point 25",
    "30-40" -> "30 to 40")."""
    if not text:
        return text
    ...
    # --- Explicit fractions: "1/2" -> "one half" --------------------------
    # Must run before the generic "x/y" or dash/slash handling below so
    # these common cases get natural words instead of "1 slash 2".
    for frac, words in _FRACTION_WORDS.items():
        text = re.sub(
            r"(?<![\d/])" + re.escape(frac) + r"(?![\d/])", words, text
        )

    # --- Times: "3:30" / "14:05" -> "3 30" / "14 05" spoken naturally -----
    # Kokoro reads "3:30" as "three colon thirty" if left alone; rewriting
    # the colon to a space lets it read it like a clock time. Restricted to
    # HH:MM shape (1-2 digit hour, exactly 2 digit minute) to avoid
    # mangling ratios or version numbers like "1:2:3".
    text = re.sub(r"\b(\d{1,2}):(\d{2})\b(?!:)", r"\1 \2", text)

    # --- Number ranges with a dash: "30-40" -> "30 to 40" -----------------
    # Must run before the general dash/hyphen cleanup and before decimal
    # point handling so it only matches digit-dash-digit, not phone
    # numbers or hyphenated words. Uses en dash and em dash too.
    text = re.sub(
        r"(?<=\d)\s*[-\u2013\u2014]\s*(?=\d)", " to ", text
    )
    ...
    # --- Percent / currency / math symbols --------------------------------
    text = re.sub(r"(\d)\s*%", r"\1 percent", text)
    text = re.sub(r"\$(\d+(?:\.\d+)?)", r"\1 dollars", text)
    text = re.sub(r"(?<=[\d\s])x(?=\s*\d)", " times ", text)  # "4x4" -> "4 times 4"
    text = re.sub(r"(?<=[\d\s])\+(?=[\d\s])", " plus ", text)
    text = re.sub(r"(?<=[\d\s])=(?=[\d\s])", " equals ", text)
    text = text.replace("&", " and ")
    text = re.sub(r"(?<=\w)@(?=\w)", " at ", text)  # "user@host" -> "user at host"
    text = text.replace("#", " number ")

    # --- Decimal numbers: "98.6" -> "98 point 6" -------------------------
    # Must run after temperature/percent/dollar rules above (which consume
    # the whole "98.6" as one number first) so a value like "98.6°F" still
    # becomes "98.6 degrees Fahrenheit" -> "98 point 6 degrees Fahrenheit",
    # rather than being split into two sentences at the "." before those
    # rules get a chance to see it as a single number.
