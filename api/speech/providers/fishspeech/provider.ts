...
import { getFishSpeechManifest } from './manifest.js';
...
export class FishSpeechProvider extends TTSProvider {
  readonly id: ProviderId = 'fishspeech';
  readonly manifest: ProviderManifest;

  constructor(private readonly runtimeRegistry: RuntimeRegistry) {
    super();
    this.manifest = getFishSpeechManifest();
  }

  /** Initializes the Fish Speech provider */
  async initialize(): Promise<void> {
    this.initialized = true;
  }

  /** Shuts down the Fish Speech provider */
  ...
  /** Synthesizes speech using Fish Speech */
  async synthesize(request: TTSRequest): Promise<TTSResponse> {
    this.assertInitialized();
    const requestId = generateRequestId();
    const runtime = this.runtimeRegistry.getByProvider(this.id);
    if (!runtime) {
      throw new SpeechError('Fish Speech runtime not available', 'RUNTIME_NOT_FOUND', this.id);
    }
    ...
    try {
      ...
      const response = await client.post<RuntimeTTSResponse>('/tts', {
        text: request.text,
        provider_id: this.id,
        voice_id: request.voiceId || defaultFishSpeechConfig.defaultVoice,
        model_id: request.modelId || defaultFishSpeechConfig.defaultModel,
      ...
      throw new SpeechError(`Fish Speech synthesis failed: ${message}`, 'SYNTHESIS_ERROR', this.id, error as Error);
    }
  }
