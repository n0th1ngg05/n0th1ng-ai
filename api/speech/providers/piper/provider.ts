...
import { getPiperManifest } from './manifest.js';
...
export class PiperProvider extends TTSProvider {
  readonly id: ProviderId = 'piper';
  readonly manifest: ProviderManifest;

  constructor(private readonly runtimeRegistry: RuntimeRegistry) {
    super();
    this.manifest = getPiperManifest();
  }

  /** Initializes the Piper provider */
  async initialize(): Promise<void> {
    this.initialized = true;
  }

  /** Shuts down the Piper provider */
  ...
  async synthesize(request: TTSRequest): Promise<TTSResponse> {
    ...
    const runtime = this.runtimeRegistry.getByProvider(this.id);
    if (!runtime) {
      throw new SpeechError('Piper runtime not available', 'RUNTIME_NOT_FOUND', this.id);
    }
    ...
    try {
      ...
      const response = await client.post<RuntimeTTSResponse>('/tts', {
        text: request.text,
        provider_id: this.id,
        voice_id: request.voiceId || defaultPiperConfig.defaultVoice,
        model_id: request.modelId || defaultPiperConfig.defaultModel,
        speed: request.speed,
      ...
      throw new SpeechError(`Piper synthesis failed: ${message}`, 'SYNTHESIS_ERROR', this.id, error as Error);
    }
  }
