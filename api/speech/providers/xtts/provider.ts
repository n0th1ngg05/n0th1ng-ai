...
import { getXTTSManifest } from './manifest.js';
...
/** XTTS TTS provider implementation */
export class XTTSProvider extends TTSProvider {
  readonly id: ProviderId = 'xtts';
  readonly manifest: ProviderManifest;

  constructor(private readonly runtimeRegistry: RuntimeRegistry) {
    super();
    this.manifest = getXTTSManifest();
  }

  /** Initializes the XTTS provider */
  async initialize(): Promise<void> {
    this.initialized = true;
  }

  /** Shuts down the XTTS provider */
  ...
  /** Synthesizes speech using XTTS */
  async synthesize(request: TTSRequest): Promise<TTSResponse> {
    this.assertInitialized();
    const requestId = generateRequestId();
    const runtime = this.runtimeRegistry.getByProvider(this.id);
    if (!runtime) {
      throw new SpeechError('XTTS runtime not available', 'RUNTIME_NOT_FOUND', this.id);
    }
    ...
    try {
      ...
      const response = await client.post<RuntimeTTSResponse>('/tts', {
        text: request.text,
        provider_id: this.id,
        voice_id: request.voiceId || defaultXTTSConfig.defaultVoice,
        model_id: request.modelId || defaultXTTSConfig.defaultModel,
      ...
      throw new SpeechError(`XTTS synthesis failed: ${message}`, 'SYNTHESIS_ERROR', this.id, error as Error);
    }
  }
